...
function canAccessSubpage(user: SessionUser, key: string) {
  ...
  const subpageRule = user.permissionConfig?.subpages?.[key];
  if (typeof subpageRule === 'boolean') return subpageRule;
  return true;
}
...
<body>
  ...
  const visibleWarehouseTabs = useMemo(() => {
    ...
    if (canAccessSubpage(user, 'inventory_stock')) items.push('stock');
    if (canAccessSubpage(user, 'inventory_query')) items.push('query');
    return items;
  }, [user]);
  const visibleAccountingTabs = useMemo(() => {
    const items: AccountingTab[] = [];
    if (canAccessSubpage(user, 'accounting_ops')) items.push('ops');
    if (canAccessSubpage(user, 'accounting_bonus')) items.push('bonus');
    if (canAccessSubpage(user, 'accounting_treasury')) items.push('treasury');
    if (canAccessSubpage(user, 'accounting_stats')) items.push('stats');
    if (canAccessSubpage(user, 'accounting_ranking')) items.push('ranking');
    if (canAccessSubpage(user, 'accounting_evaluation')) items.push('evaluation');
    return items;
  }, [user]);
  ...
  ].filter((item) => canAccessNav(user, item.key)), [user]);
  const currentNavItem = navItems.find((item) => item.key === active) || visibleNavItems[0] || navItems[0];
  ...
  }, [active, warehouseTab, visibleWarehouseTabs]);
  ...
  }, [active, accountingTab, visibleAccountingTabs]);
