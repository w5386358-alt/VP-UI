    ...
    <div className="vp-shell">
      <aside className="vp-sidebar">
        <div className="vp-brand">
          <div className="vp-brand-title">Velvet Pulse</div>
        </div>

        <div className="vp-user-card">
          <div className="vp-avatar">{String(user.name || 'U').slice(0, 1)}</div>
          <div>
            <p className="vp-user-name">{user.name}</p>
            <p className="vp-user-meta">ID: {user.loginId} | {ROLE_LABEL[user.role]}</p>
          </div>
        </div>
          ...
          {visibleNavItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`vp-nav-item ${active === item.key ? 'active' : ''}`}
              >
                <Icon className="vp-nav-icon" />
        ...
        <div className="vp-sidebar-bottom">
          <button type="button" className="vp-side-button">系統公告</button>
          <button type="button" className="vp-logout-button">
      ...
      <div className="vp-main-wrap">
        <header className="vp-topbar">
          <div className="vp-topbar-left">
            <h2 className="vp-topbar-title">{visibleNavItems.find((item) => item.key === active)?.label || '總覽'} / 操作區</h2>
          ...
          <div className="vp-topbar-actions">
            <button type="button" className="vp-refresh-button" onClick={() => void loadFirebaseData()}>
              <RefreshCw className="small-icon" />
              重新整理
            </button>
            <div className="vp-top-icons">
              <Bell className="small-icon" />
              <Settings className="small-icon" />
            </div>
          </div>
        </header>
        ...
        <main className="vp-main-content">
          {booting ? (
            ...
            <>
              <section className="vp-welcome-banner">
                <div className="vp-welcome-copy">
                  <h3>歡迎回來，{user.name}。</h3>
                  <p>
                    <span className={`vp-status-dot ${firebaseReady ? 'ok' : 'warn'}`}></span>
                    {firebaseReady ? '系統已連線至 Firebase 真實資料。' : '目前使用本地資料。'}
                  </p>
                </div>
                <div className="vp-welcome-icon">
                  <Sparkles className="vp-welcome-symbol" />
                </div>
              </section>

              {active === 'dashboard' && (
                <>
                  <section className="vp-stats-grid">
                    {[
                      { title: '今日實收', value: `$${Number(grandTotal || 0).toLocaleString('zh-TW')}`, sub: '目前購物車試算', icon: Wallet, tone: 'pink' },
                      { title: '待收款', value: `$${Number(paymentQueue.filter((item) => item.paymentStatus !== '已收款').reduce((sum, item) => sum + Number(item.amount || 0), 0)).toLocaleString('zh-TW')}`, sub: '依訂單記錄統計', icon: Receipt, tone: 'rose' },
                      { title: '低庫存商品', value: `${lowStockCount} 件`, sub: '需優先補貨', icon: Boxes, tone: 'danger' },
                      { title: '今日入出庫', value: `${warehouseRecentLogs.length} 次`, sub: '依最新異動紀錄', icon: ArrowUpRight, tone: 'mint' },
                    ].map((card) => {
                      const Icon = card.icon;
                      return (
                        <div key={card.title} className={`vp-stat-card ${card.tone}`}>
                          <div className="vp-stat-top">
                            <span className="vp-stat-icon"><Icon className="small-icon" /></span>
                          </div>
                          <div>
                            <p className="vp-stat-title">{card.title}</p>
                            <p className="vp-stat-value">{card.value}</p>
                            <p className="vp-stat-sub">{card.sub}</p>
                          </div>
                        </div>
                      );
                    })}
                  ...
                  <section className="vp-workflow-block">
                    <h4 className="vp-section-heading">核心工作流程</h4>
                    <div className="vp-workflow-grid">
                      {workflowCards.slice(0, 4).map((card) => {
                        const Icon = card.icon;
                        return (
                          <div key={card.title} className="vp-workflow-card">
                            <div className="vp-workflow-head">
                              <div>
                                <div className="vp-workflow-title-row">
                                  <Icon className="vp-workflow-icon" />
                                  <h5>{card.title}</h5>
                                </div>
                                <p>{card.desc}</p>
                              </div>
                              <button type="button" className="vp-link-button">
                                <ChevronRight className="small-icon" />
                              </button>
                            </div>
                            <div className="vp-workflow-list-grid">
                              <ul>
                                {card.bullets.slice(0, 2).map((bullet) => <li key={bullet}>{bullet}</li>)}
                              </ul>
                              <ul>
                                {card.bullets.slice(2, 4).map((bullet) => <li key={bullet}>{bullet}</li>)}
                              </ul>
                            </div>
                          </div>
                        );
              ...
              <section className="vp-module-stage">
                {active === 'dashboard' && (
