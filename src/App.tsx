...
type WarehouseQueryMode = '商品條碼' | 'QR 身分識別' | '訂單編號';
type WarehouseQueryResult = { title: string; desc: string; meta: string[] };
...
export default function App() {
  ...
  const [shippingDoneOrders, setShippingDoneOrders] = useState<string[]>([]);
  const [warehouseQueryMode, setWarehouseQueryMode] = useState<WarehouseQueryMode>('商品條碼');
  const [warehouseQueryInput, setWarehouseQueryInput] = useState('');
  const [warehouseQueryResult, setWarehouseQueryResult] = useState<WarehouseQueryResult | null>({
    title: '商品條碼模式',
    desc: '商品名稱、商品條碼、總庫存、最近入庫時間、QR(A)*2 / QR(B)*1',
    meta: ['E401', '女神酵素液', '最近入庫 2026/03/31 10:45'],
  });
  ...
  const selectedWarehouseShippingStatus = selectedWarehouseOrder ? (shippingDoneOrders.includes(selectedWarehouseOrder.orderNo) ? '已出貨' : selectedWarehouseOrder.shippingStatus) : '-';
  const selectedWarehousePaymentStatus = selectedWarehouseOrder?.paymentStatus || '-';
  const selectedWarehouseTrackingNo = selectedWarehouseOrder ? `TRK-${selectedWarehouseOrder.orderNo.slice(-4)}` : '-';
  ...
  function selectWarehouseOrder(orderNo: string, source: 'scan' | 'sheet' = 'sheet') {
    setSelectedWarehouseOrderNo(orderNo);
    setWarehouseNotice({ text: source === 'scan' ? '✅ 已帶入出貨單' : '✅ 已切換訂單', tone: 'success' });
  }

  function completeWarehouseShipping() {
    if (!selectedWarehouseOrder) return;
    if (!['已收款', '免收款'].includes(selectedWarehouseOrder.paymentStatus)) {
      setWarehouseNotice({ text: '❌ 未收款不可出貨', tone: 'danger' });
      return;
    }
    if (shippingDoneOrders.includes(selectedWarehouseOrder.orderNo)) {
      setWarehouseNotice({ text: '❌ 此訂單已出貨', tone: 'danger' });
      return;
    }
    setShippingDoneOrders((prev) => [...prev, selectedWarehouseOrder.orderNo]);
    setWarehouseNotice({ text: `✅ 已出貨：${selectedWarehouseOrder.orderNo}`, tone: 'success' });
  }

  function printWarehouseSheet() {
    if (!selectedWarehouseOrder) return;
    setWarehouseNotice({ text: `✅ 已產生出貨單：${selectedWarehouseOrder.orderNo}`, tone: 'neutral' });
  }

  function runWarehouseQuery(mode = warehouseQueryMode, raw = warehouseQueryInput) {
    const q = raw.trim().toLowerCase();
    if (!q) {
      setWarehouseQueryResult({
        title: '查詢結果展示位',
        desc: '請輸入商品條碼 / QR 身分識別 / 訂單編號。',
        meta: ['支援精準查詢', '可接掃碼帶入', '保留最近 20 筆'],
      });
      setWarehouseNotice({ text: '❌ 尚未輸入查詢條件', tone: 'danger' });
      return;
    }

    if (mode === '商品條碼') {
      const hit = stockSnapshot.find((item) => item.code.toLowerCase().includes(q) || item.name.toLowerCase().includes(q));
      if (hit) {
        setWarehouseQueryResult({
          title: '商品條碼模式',
          desc: `${hit.name} / ${hit.code} / 目前庫存 ${hit.stock} / ${hit.qr}`,
          meta: [`安全庫存 ${hit.safe}`, `狀態 ${hit.status}`, '最近入庫 2026/03/31 10:45'],
        });
        setWarehouseNotice({ text: `✅ 已查到商品：${hit.code}`, tone: 'success' });
        return;
      }
    }
    ...
    if (mode === 'QR 身分識別') {
      const qrMap = [
        { key: 'qr(a)', title: 'QR 模式', desc: '女神酵素液 / E401 / QR(A) 目前庫存 18', meta: ['入庫時間 2026/03/31 10:45', '入庫人員 王小婷', '商品條碼 E401'] },
        { key: 'qr(b)', title: 'QR 模式', desc: '女神酵素液 / E401 / QR(B) 目前庫存 18', meta: ['入庫時間 2026/03/31 10:45', '入庫人員 王小婷', '商品條碼 E401'] },
        { key: 'qr(p1)', title: 'QR 模式', desc: '瞬白激光精華4G / P301 / QR(P1) 目前庫存 6', meta: ['入庫時間 2026/03/30 16:20', '入庫人員 陳小安', '商品條碼 P301'] },
      ]
      const hit = qrMap.find((item) => q.includes(item.key) || item.key.includes(q));
      if (hit) {
        setWarehouseQueryResult({ title: hit.title, desc: hit.desc, meta: hit.meta });
        setWarehouseNotice({ text: '✅ 已查到 QR 身分識別', tone: 'success' });
        return;
      }
    }

    const orderHit = shippingQueue.find((item) => item.orderNo.toLowerCase().includes(q));
    if (mode === '訂單編號' && orderHit) {
      const done = shippingDoneOrders.includes(orderHit.orderNo);
      setWarehouseQueryResult({
        title: '訂單模式',
        desc: `${orderHit.orderNo} / ${orderHit.customer} / ${done ? '已出貨' : orderHit.shippingStatus}`,
        meta: [orderHit.paymentStatus, `${orderHit.itemCount} 件`, done ? '已完成出貨作業' : '待進一步處理'],
      });
      setWarehouseNotice({ text: `✅ 已查到訂單：${orderHit.orderNo}`, tone: 'success' });
      return;
    }
    ...
    setWarehouseQueryResult({
      title: '查無資料',
      desc: '目前沒有符合條件的查詢結果。',
      meta: ['請檢查商品條碼', '請檢查 QR 身分識別', '請檢查訂單編號'],
    });
    setWarehouseNotice({ text: '❌ 查無符合資料', tone: 'danger' });
  }

  function scanIntoWarehouseQuery() {
    const sample = selectedWarehouseOrder?.orderNo || 'VP20260331-001';
    setWarehouseQueryMode('訂單編號');
    setWarehouseQueryInput(sample);
    runWarehouseQuery('訂單編號', sample);
  }
          ...
          <>
            ...
            {active === 'inventory' && (
              <>
                ...
                {warehouseNotice && (
                  <div className={`card product-notice-banner ${warehouseNotice.tone}`}>
                      ...
                      <div className="card order-panel">
                        ...
                        <div className="shipping-queue">
                          {shippingQueue.map((item) => {
                            const isSelected = item.orderNo === selectedWarehouseOrderNo;
                            const isDone = shippingDoneOrders.includes(item.orderNo);
                            return (
                              <div key={item.orderNo} className={`shipping-row shipping-row-selectable ${isSelected ? 'selected' : ''}`}>
                                <div>
                                ...
                                <div className="shipping-actions">
                                  <span className={`badge ${isDone ? 'badge-success' : item.urgency === 'high' ? 'badge-danger' : 'badge-neutral'}`}>{isDone ? '已出貨' : item.shippingStatus}</span>
                                  <button type="button" className="ghost-button compact-btn" onClick={() => selectWarehouseOrder(item.orderNo, 'scan')}>
                                    <QrCode className="small-icon" />掃碼出貨
                                  </button>
                                  <button type="button" className="ghost-button compact-btn" onClick={() => selectWarehouseOrder(item.orderNo)}>
                                    <Eye className="small-icon" />查看出貨單
                                  </button>
                                </div>
                              </div>
                            );
                      ...
                      <div className="warehouse-detail-grid">
                        ...
                        <div className="card warehouse-detail-card">
                          ...
                          <div className="warehouse-form-grid">
                            ...
                            <div className="fake-field"><span>出貨狀態</span><strong>{selectedWarehouseShippingStatus}</strong></div>
                            <div className="fake-field"><span>收款狀態</span><strong>{selectedWarehousePaymentStatus}</strong></div>
                            <div className="fake-field"><span>追蹤編號</span><strong>{selectedWarehouseTrackingNo}</strong></div>
                            <div className="fake-field wide"><span>掃碼結果</span><strong>{shippingDoneOrders.includes(selectedWarehouseOrder?.orderNo || '') ? '✅ 商品條碼 + QR 身分識別比對完成，可追蹤已出貨紀錄' : '商品條碼 + QR 身分識別比對成功後才放行'}</strong></div>
                          </div>
                          <div className="accounting-action-row">
                            <button type="button" className="primary-button" onClick={completeWarehouseShipping}><Truck className="small-icon" />完成出貨</button>
                            <button type="button" className="ghost-button" onClick={printWarehouseSheet}><Receipt className="small-icon" />列印出貨單</button>
                          </div>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="card warehouse-tool-card">
                        <div className="warehouse-card-head">
                          <div>
                            ...
                            <div className="flow-desc">這版已補前端互動，查詢、掃碼帶入、結果切換都可直接操作。</div>
                          </div>
                        ...
                        <div className="form-grid two-col form-gap-top">
                          ...
                          <label className="field-card">
                            <span className="field-label"><ClipboardList className="small-icon" />查詢模式</span>
                            <select value={warehouseQueryMode} onChange={(e) => setWarehouseQueryMode(e.target.value as WarehouseQueryMode)}>
                              <option value="商品條碼">商品條碼</option>
                              <option value="QR 身分識別">QR 身分識別</option>
                              <option value="訂單編號">訂單編號</option>
                            </select>
                          </label>
                          <div className="fake-field">
                            <span>最近紀錄</span>
                            <strong>保留 20 筆</strong>
                          </div>
                        </div>
                        <div className="accounting-action-row">
                          <button type="button" className="primary-button" onClick={() => runWarehouseQuery()}><Search className="small-icon" />立即查詢</button>
                          <button type="button" className="ghost-button" onClick={scanIntoWarehouseQuery}><QrCode className="small-icon" />掃碼帶入</button>
                        </div>
                      </div>
                      ...
                      <div className="card warehouse-tool-card">
                        <div className="warehouse-card-head">
                          <div>
                            ...
                            <div className="flow-desc">依你之前定義的顯示規則，現在已可依查詢模式切換內容。</div>
                          </div>
                          <History className="small-icon" />
                        </div>
                        <div className="warehouse-result-list">
                          <div className="warehouse-result-item">
                            <div className="warehouse-result-title">{warehouseQueryResult?.title || '查詢結果展示位'}</div>
                            <div className="warehouse-result-desc">{warehouseQueryResult?.desc || '尚未查詢'}</div>
                          </div>
                          {(warehouseQueryResult?.meta || []).map((item) => (
                            <div key={item} className="warehouse-result-item compact-result-item">
                              <div className="warehouse-result-desc">{item}</div>
                            </div>
                          ))}
                        </div>
                      </div>
