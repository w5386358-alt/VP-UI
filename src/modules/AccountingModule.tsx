...
import { CreditCard, BarChart3, Trophy, Search, CalendarRange, Truck, Receipt, Wallet, BadgePercent, FileText, RefreshCw, ArrowUpRight, Sparkles, ShieldCheck, Clock3, ChevronLeft, ChevronRight, ClipboardCheck, Layers3, PieChart, GitBranchPlus, Activity } from 'lucide-react';
  ...
  return (
    <>
      ...
      <section className="accounting-shell-v2">
        ...
        {accountingTab === 'stats' && (
          <section className="accounting-stats-layout-v2 accounting-report-layout-v3">
            <div className="accounting-stats-main">
              ...
              <div className="accounting-report-feature-grid">
                <div className="card order-panel accounting-donut-card">
                  <div className="panel-head"><div><div className="panel-title">收款 / 退款 / 支出占比</div><div className="panel-desc">先以視覺模組呈現，後續再接真實公式。</div></div><span className="badge badge-soft">圓餅</span></div>
                  <div className="accounting-donut-wrap">
                    <div className="accounting-donut-chart">
                      <div className="accounting-donut-ring" />
                      <div className="accounting-donut-center"><strong>營運</strong><span>結構圖</span></div>
                    </div>
                    <div className="accounting-donut-legend">
                      <div className="accounting-legend-row"><span className="legend-dot revenue" />已收款 68%</div>
                      <div className="accounting-legend-row"><span className="legend-dot refund" />退款中 17%</div>
                      <div className="accounting-legend-row"><span className="legend-dot expense" />支出 15%</div>
                    </div>
                  </div>
                </div>

                <div className="card order-panel accounting-tree-card">
                  <div className="panel-head"><div><div className="panel-title">營運結構分流</div><div className="panel-desc">用樹狀流程先整理報表方向。</div></div><span className="badge badge-role">樹狀</span></div>
                  <div className="accounting-tree-flow">
                    <div className="tree-root"><GitBranchPlus className="small-icon" /><span>營運報表</span></div>
                    <div className="tree-branches">
                      <div className="tree-branch-card income"><span>收入</span><strong>已收款 / 待收款</strong></div>
                      <div className="tree-branch-card refund"><span>退款</span><strong>退款中 / 已退款</strong></div>
                      <div className="tree-branch-card expense"><span>支出</span><strong>進貨 / 運費 / 雜支</strong></div>
                    </div>
                  </div>
                </div>
              </div>

              <div className="card order-panel accounting-trend-card-v2 accounting-trend-card-v3">
                <div className="panel-head"><div><div className="panel-title">區間營收趨勢</div><div className="panel-desc">保留原本趨勢感，再強化圖像層次。</div></div><span className="badge badge-soft">趨勢</span></div>
                <div className="accounting-trend-chart-v3">
                  {accounting趨勢Bars.map((item: any) => (
                    <div key={item.label} className="accounting-trend-bar-col">
                      <div className="accounting-trend-bar-shell"><div className="accounting-trend-bar-fill" style={{ height: `${Math.max(12, item.width)}%` }} /></div>
            ...
            <div className="accounting-stats-side">
              <div className="card accounting-insight-card-v2 accounting-insight-card-v3">
                <div className="accounting-insight-icon"><PieChart className="small-icon" /></div>
                <div className="accounting-insight-title">營運摘要</div>
                <div className="stack-list compact"><div>圖表先看結構</div><div>之後再接真實邏輯</div><div>方便後續補收入 / 退款 / 支出</div></div>
              </div>
              <div className="card accounting-mini-kpi-card">
                <div className="accounting-mini-kpi-head"><Activity className="small-icon" /><span>本版設計方向</span></div>
                <div className="accounting-mini-kpi-list">
                  <div><strong>圓餅圖</strong><span>快速看比例</span></div>
                  <div><strong>樹狀圖</strong><span>看資金流向</span></div>
                  <div><strong>柱狀圖</strong><span>看區間變化</span></div>
                </div>
              </div>
            </div>
          </section>
