...
=== 第三階段：GAS 同步中心（新試算表版） ===
- 已新增同步中心骨架：src/firebase/syncMapping.ts
- 已新增同步動作定義：src/firebase/syncCenter.ts
- 已新增第三階段說明：GAS_SYNC_CENTER_第三階段.txt
- 同步來源已固定為新的試算表，不再沿用舊版試算表
- 目前先建立同步規則與按鈕骨架，不在前端直接做危險批量寫入
